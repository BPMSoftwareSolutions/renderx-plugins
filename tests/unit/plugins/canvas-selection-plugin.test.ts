...
describe("RenderX Canvas Selection Plugin", () => {
  ...
  describe("handlers", () => {
    ...
    test("handleSelect calls callback and returns selected", () => {
      ...
      const ctx: any = base();
      const res = plugin.handlers.handleSelect(
        { elementId: "id1", onSelectionChange },
        ctx
      );
    ...
    test("handleFinalize clears selection when flag is true", () => {
      const onSelectionChange = jest.fn();
      const ctx: any = base();
      const res = plugin.handlers.handleFinalize(
        { elementId: "id1", clearSelection: true, onSelectionChange },
        ctx
      );
