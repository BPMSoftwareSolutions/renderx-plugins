...
describe("Canvas UI Plugin - selection click → visual tools", () => {
  test("clicking the element plays Canvas.component-select-symphony and logs signature", async () => {
    ...
    const node = {
      ...
      component: {
        ...
        integration: {
          canvasIntegration: { defaultWidth: 100, defaultHeight: 20 },
        },
      },
    };
    ...
    expect(
      logs.some((l) =>
        l.includes(
          "PluginInterfaceFacade.play(): Canvas.component-select-symphony"
        )
      )
    ).toBe(true);
  ...
  test("CanvasPage listens for selection update and renders overlay for selected node", () => {
    ...
    const node = {
      id: "rx-comp-button-sel2",
      cssClass: "rx-comp-button-sel2",
      type: "button",
      position: { x: 0, y: 0 },
      component: {
        metadata: { name: "Button", type: "button" },
    ...
    const overlay = ReactStub.createElement("div", {
