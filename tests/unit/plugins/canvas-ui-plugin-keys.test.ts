  ...
  test("renders nodes with stable keys and warns/skips when missing id/elementId/cssClass", () => {
    ...
    try {
      ...
      const goodA = render({
        ...
        component: {
          ui: {
            template: '<button class="rx-button">A</button>',
      ...
      const goodB = render({
        ...
        component: {
          ui: {
            template: '<button class="rx-button">B</button>',
            styles: { css: ".rx-button{color:#111}" },
          },
        },
      });
      const bad = render({
        ...
        component: {
          ui: {
            template: '<button class="rx-button">X</button>',
            styles: { css: ".rx-button{color:#222}" },
          },
        },
      });
      ...
      const out = [
        mapWithKeys({ id: "id-a", cssClass: "rx-comp-a" }),
        mapWithKeys({ id: "id-b", cssClass: "rx-comp-b" }),
        mapWithKeys({ id: "", cssClass: "" }),
      ];
