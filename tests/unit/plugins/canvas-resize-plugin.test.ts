...
describe("RenderX Canvas Resize Plugin", () => {
  ...
  describe("handlers", () => {
    ...
    test("handleResizeStart seeds payload", () => {
      const ctx: any = base();
      const res = plugin.handlers.handleResizeStart(
        {
          elementId: "a",
          handle: "se",
          start: { x: 1, y: 2 },
          tools: { resize: { constraints: { min: { w: 10, h: 10 } } } },
          startBox: { x: 0, y: 0, w: 100, h: 50 },
        },
        ctx
      );
    ...
    test("handleResizeMove applies deltas and constraints for SE handle", () => {
      ...
      ctx.payload.resize = {
        startBox: { x: 0, y: 0, w: 100, h: 50 },
        constraints: { min: { w: 30, h: 20 } },
      };
      const res = plugin.handlers.handleResizeMove(
        {
          elementId: "a",
          handle: "se",
          delta: { dx: 10, dy: 5 },
