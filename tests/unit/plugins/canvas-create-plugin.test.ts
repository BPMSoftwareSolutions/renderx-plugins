...
describe("RenderX Canvas Create Plugin", () => {
  ...
  describe("handlers", () => {
    ...
    test("createCanvasComponent returns created payload with id/cssClass/type/position and invokes callback", () => {
      ...
      const c: any = ctx();
      const res = plugin.handlers.createCanvasComponent(
        { component, position, onComponentCreated },
        c
      );
      ...
      expect(onComponentCreated).toHaveBeenCalledWith(
        expect.objectContaining({ id: res.id, cssClass: res.cssClass })
      );
    });
  });
});
