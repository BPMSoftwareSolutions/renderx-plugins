...
describe("Canvas UI - resize via overlay handle updates overlay + commits instance CSS (SE handle)", () => {
  test("pointerdown/move/up (SE) grows width/height from defaults (120x40) to (140x50)", async () => {
    ...
    expect(strip(overlayTag!.textContent || "")).toContain(strip(`.rx-overlay-${node.id}{position:absolute;left:10px;top:20px;width:120px;height:40px;z-index:10;}`));
    ...
    await handle.props.onPointerDown({ clientX: 0, clientY: 0, stopPropagation() {} });
